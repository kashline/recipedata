...
async function createRecipeBatch() {
    ...
    const category = recipeCategories[Math.round(Math.random() * recipeCategories.length)];
    const region = recipeRegions[Math.round(Math.random() * recipeRegions.length)];
    const now = new Date();
            ...
            if (file.purpose === "batch_output") {
                ...
                fileContents.map(async (recipe) => {
                    if (typeof recipe === "string") {
                        ...
                        await fileJson.response.body.choices.map(async (choice) => {
                            const recipe = RecipeZodel.parse(await JSON.parse(choice.message.content));
...
async function generateMissingImages() {
    ...
    try {
        const image = await openai.images.generate({
            model: "dall-e-2",
            prompt: `A realistic photograph of ${recipe.dataValues.title} freshly prepared and professionally presented.`,
            n: 1,
            size: "1024x1024",
            response_format: "url",
        });
        await retrieveAndStoreImage(image.data[0].url, `${recipe?.dataValues.title}`, filename);
        console.log(`Generated image and updated database.  Deleting local file and sleeping for 10 seconds.`);
        fs.unlink(`./${filename}`, (err) => { if (err) {
            console.log(err);
        } });
        await sleep(10000);
...
async function retrieveAndStoreImage(uri, recipeTitle, filename) {
    try {
    ...
    try {
        ...
        if (caught instanceof S3ServiceException &&
            ...
            console.error(`Error from S3 while uploading object to ${bucketConfig.Bucket}. \
The object was too large. To upload objects larger than 5GB, use the S3 console (160GB max) \
or the multipart upload API (5TB max).`);
        }
        else if (caught instanceof S3ServiceException) {
            console.error(`Error from S3 while uploading object to ${bucketConfig.Bucket}.  ${caught.name}: ${caught.message}`);
        }
        else {
            throw caught;
        }
...
async function updateImageUrl(recipeTitle, url) {
